...
def humanTurn(gs, whitePlayer, blackPlayer):
    return (gs.whiteToMove and whitePlayer == "person") or ((not gs.whiteToMove) and blackPlayer == "person")

def boardFlipped(gs, whitePlayer, blackPlayer):
    if humanTurn(gs, whitePlayer, blackPlayer):
        return not gs.whiteToMove  
    return whitePlayer == "ai"     
    ...
    while running:
        ...
        for e in p.event.get():
            if e.type == p.QUIT:
                ...
                screenCol = location[0] // SQ_SIZE
                screenRow = (location[1] - TOP_BAR) // SQ_SIZE

                flipped = boardFlipped(
                    gs,
                    "person" if playerOne else "ai",
                    "person" if playerTwo else "ai"
                )
        ...
        drawGameState(screen, gs, validSquares, flipped, evalScore)
        clock.tick(MAX_FPS)
        p.display.flip()
...
def drawGameState(screen, gs, validSquares, flipped, evalScore):
    drawBoard(screen)
    highlightSquares(screen, validSquares, flipped)
...
def drawPieces(screen, board, flipped):
    for r in range(DIMENSION):
        for c in range(DIMENSION):
            piece = board[r][c]
            if piece != "--":
                drawRow = 7 - r if flipped else r
                drawCol = 7 - c if flipped else c

                screen.blit(
                    IMAGES[piece],
                    p.Rect(
                        drawCol * SQ_SIZE,
                        drawRow * SQ_SIZE + TOP_BAR,
                        SQ_SIZE,
                        SQ_SIZE
                    )
                )
...
def highlightSquares(screen, validSquares, flipped):
    highlight = p.Surface((SQ_SIZE, SQ_SIZE))
    highlight.set_alpha(120)
    highlight.fill(p.Color("yellow"))

    for row, col in validSquares:
        drawRow = 7 - row if flipped else row
        drawCol = 7 - col if flipped else col

        screen.blit(
            highlight,
            (drawCol * SQ_SIZE, drawRow * SQ_SIZE + TOP_BAR)
        )
